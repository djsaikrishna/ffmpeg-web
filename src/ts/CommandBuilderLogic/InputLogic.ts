 ...
 */
export default async function InputLogic(generalFiles: File[], handle?: FileSystemDirectoryHandle) {
    ...
    for (const files of FileDivider(generalFiles)) {
        ...
        for (let i = 0; i < getVal.length; i++) {
            ...
            function patch() {
                ...
                if (!isNaN(outputNumber) && files[outputNumber]) {
                    getVal[i] = `${getVal[i].substring(0, matchedStr.index)}${FFmpegFileNameHandler(files[outputNumber])}${getVal[i].substring(matchedStr.index + "$input[".length + addTo + 1)}`;
